...
## CUDA percentage-change backend

### Prerequisites
- CUDA Toolkit with `nvcc` in `PATH` (for GPU build path).
- If `nvcc` is missing, `zig build` still succeeds and uses CPU fallback stubs.

### Build
- `zig build -Dtarget=native -Dcpu=native`

### Runtime backend selection
- `./zig-out/bin/MicroRush --force-cpu` to always use CPU percentage-change compute.
- `./zig-out/bin/MicroRush --force-gpu` to require GPU; startup exits if CUDA is unavailable.
- Default (no flag) is auto-detect.

### Verify backend at startup
Check logs for:
- `CUDA devices detected: X`
- `Backend selected: GPU` or `Backend selected: CPU`
